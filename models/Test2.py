...
class SparseVariableRouter(nn.Module):
    def __init__(self, num_vars, hidden_dim=16, temp=1.0):
...
class HTMBlock(nn.Module):
    ...
    # def forward(self, x, use_t1=False, use_t2=True, use_t3=False):  # [B, L, N]
    #     B, L, N = x.shape
    #     x_ = x.permute(0, 2, 1)  # [B, N, L]
    #
    #     # 各分支输出（若未启用则为 0）
    #     t1_out = self.t1(x_) if use_t1 else 0
    #     t2_out = self.t2(x_) if use_t2 else 0
    #     t3_out = self.t3(x_) if use_t3 else 0
    #
    #     # gate 输入：mean, std, pred_len
    #     mean = x_.mean(dim=-1)  # [B, N]
    #     std = x_.std(dim=-1)  # [B, N]
    #     pred_len_feat = torch.full((B, 1), L / 1000.0, device=x.device)  # [B, 1]
    #     gate_input = torch.cat([mean, std, pred_len_feat], dim=-1)  # [B, 2N+1]
    #
    #     weights = self.gate_mlp(gate_input)  # [B, 3]
    #     w1 = weights[:, 0:1, None]
    ...
    #
    #     fused = self.proj(fused).permute(0, 2, 1)  # [B, L, N]
    #     return self.norm(self.dropout(fused + x))  # 残差连接 + LayerNorm
        ...
        self.encoder = Encoder([
            EncoderLayer(
                UnifiedRingStarBlock(
                    ...
                    use_multi_center=getattr(configs, 'use_multi_center', True),
