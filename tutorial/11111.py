...
class RingStarBlock(nn.Module):
    def __init__(self, d_core=64, kernel_size=3, dilation=1, num_vars=None, use_multi_center=True, in_dim=None):
        ...
        # 如果输入维度 != num_vars，先加映射
        if in_dim is not None and in_dim != num_vars:
            self.in_proj = nn.Linear(in_dim, num_vars)
        else:
            self.in_proj = None
    ...
    def forward(self, x):  # x: [B, L, N]
        ...
        if self.in_proj is not None:
            x = self.in_proj(x)  # [B, L, num_vars]

        #print("RingStarBlock forward called")  # 如果能打印，说明正常
        B, L, N = x.shape
        device = x.device
